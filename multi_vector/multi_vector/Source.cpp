...
public:
	//	template<T>
	cvector<T>(int n = 0) : n_(n) { this->vec.resize(n); }
	void stretch(int n)
...
public:
	...
	cvector<T>(const cvector<T> & obj) : n_(obj.n_) { this->vec = obj.vec; }
	cvector<T>(const initializer_list<T> & il) : n_(il.size()) { this->vec = vector<T>(il); }
